...
/* Initial codul continea poantori la int*, dar am modificat pentru 
 * a avea struct heapElement. Ne ajuta mai mult pe k-way-sort */
...
struct heapStruct* initHeap() {
    ...
    h->capacity = SIZE;
    h->heaparray = (struct heapElement*)malloc(sizeof(struct heapElement)*(SIZE+1));
    h->size = 0;
    return h;
}

int heapSize(struct heapStruct *h) {
	return h->size;
}
...
struct heapStruct * initHeapfromArray(int* values, int length) {
    ...
    h->heaparray = (struct heapElement*)malloc(sizeof(struct heapElement)*(length+1));
    
    // Just copy the values into our array.
    for (i=1; i<=length; i++) {
        h->heaparray[i].value = values[i-1];
        h->heaparray[i].list_index = i-1;
	}
...
void percolateDown(struct heapStruct *h, int index) {
    ...
    if ((2*index+1) <= h->size) {
        ...
        min = minimum(h->heaparray[2*index].value, 2*index, h->heaparray[2*index+1].value, 2*index+1);
        
      // If this value is less than the current value, then we need to move
      // our current value down the heap.  
        if (h->heaparray[index].value > h->heaparray[min].value) {
            swap(h, index, min);
    ...
    // Case where our current element has exactly one child, a left child.
        ...
        // is a leaf. 
        if (h->heaparray[index].value > h->heaparray[2*index].value) 
...
void percolateUp(struct heapStruct *h, int index) {
    ...
    if (index > 1) {
        ...
        if (h->heaparray[index/2].value > h->heaparray[index].value) {
...
void insert(struct heapStruct *h, int value, int index) {
    ...
    if (h->size == h->capacity) {
        ...
        temp = (struct heapElement*)malloc(sizeof(struct heapElement)*h->capacity+1);
      
        // Copy all the items over.
        for (i=1; i<=h->capacity; i++) {
            temp[i].value = h->heaparray[i].value;
			//temp[i].list_index = h->heaparray[i].list_index;
		}
    ...
    h->size++;
    h->heaparray[h->size].value = value;
    h->heaparray[h->size].list_index = index;
    percolateUp(h, h->size);
}

struct heapElement removeMin(struct heapStruct *h) {

    struct heapElement retval;
    
    // We can only remove an element, if one exists in the heap!
    if (h->size > 0) {

        // This is where the minimum is stored.
        retval.value = h->heaparray[1].value;
        retval.list_index = h->heaparray[1].list_index;
        
        // Copy the last value into this top slot.
        h->heaparray[1].value = h->heaparray[h->size].value;
        h->heaparray[1].list_index = h->heaparray[h->size].list_index;
...
void swap(struct heapStruct *h, int index1, int index2) {
    int temp = h->heaparray[index1].value;
    h->heaparray[index1].value = h->heaparray[index2].value;
    h->heaparray[index2].value = temp;
    
    int tempIndex = h->heaparray[index1].list_index;
    h->heaparray[index1].list_index = h->heaparray[index2].list_index;
    h->heaparray[index2].list_index = tempIndex;
}
